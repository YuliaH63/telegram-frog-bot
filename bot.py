...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_text == "⚡ Энергоматрица":
        ...
        if has_energy_access(user_id):
            ...
            await update.message.reply_text(
                ...
                "Чтобы продолжить, можно получить новый расчёт "
                "или пакет расчётов.\n\n"
                "Для пополнения баланса напишите мне в личку "
                "@YHayat 🐸",
                reply_markup=reply_markup
            )
    ...
    if (
     ...
     # 🚫 В состоянии подтверждения цели принимаем только кнопки
    if state == "ENERGY_MATRIX_CONFIRM_GOAL":
